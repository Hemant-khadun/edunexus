<div>
    <div class=" flex items-center justify-between pb-6">
        <div>
            <h2 class="text-gray-600 dark:text-gray-200 font-semibold">Topics</h2>
            <span class="text-xs dark:text-slate-400">All topic items</span>
        </div>
                ...
                <div class="lg:ml-40 ml-10 space-x-8">
                    @if(Auth::user()->getRoleAttribute() == 'Administrator' || auth()->user()->getRoleAttribute() == 'Tutor')
                        <a href="{{route('topic.create')}}">
                            <button type="button" class="mt-6 text-white bg-gradient-to-r from-teal-400 via-teal-500 to-teal-600 hover:bg-gradient-to-br focus:ring-4 focus:outline-none focus:ring-teal-300 dark:focus:ring-teal-800 shadow-lg shadow-teal-500/50 dark:shadow-lg dark:shadow-teal-800/80 font-medium rounded-lg text-sm px-5 py-2.5 text-center me-2 mb-2">Add new topic</button>
                        </a>
                    @endif
                </div>
            </div>
                    ...
                    <table class="min-w-full leading-normal">
                        ...
                        @foreach ($topics as $topic)
                            ...
                            <tr>
                                    ...
                                    <div class="flex items-center">
                                            <div class="ml-3">
                                                <p class="text-gray-900 whitespace-no-wrap dark:text-slate-300">
                                                    @if (isset($topic->name)) {{$topic->name}} @endif
                                                </p>
                                            </div>
                                        </div>
                                </td>
                                ...
                                <td class="px-5 py-5 border-b border-gray-200 bg-white dark:bg-slate-700 dark:border-gray-800 text-sm">
                                    <p class="text-gray-900 whitespace-no-wrap dark:text-slate-300">@if (isset($topic->program->title)) {{$topic->program->title}} @endif</p>
                                </td>

                                <td class="px-5 py-5 border-b border-gray-200 bg-white dark:bg-slate-700 dark:border-gray-800 text-sm">
                                    <p class="text-gray-900 whitespace-no-wrap dark:text-slate-300">@if (isset($topic->subject->title)) {{$topic->subject->title}} @endif</p>
                                </td>
                                <td class="px-5 py-5 border-b border-gray-200 bg-white dark:bg-slate-700 dark:border-gray-800 text-sm">
                                    <p class="text-gray-900 whitespace-no-wrap dark:text-slate-300">@if (isset($topic->course->title)) {{$topic->course->title}} @endif</p>
                                </td>

                                <td class="px-5 py-5 border-b border-gray-200 bg-white dark:bg-slate-700 dark:border-gray-800 text-sm">
                                    <p class="text-gray-900 whitespace-no-wrap dark:text-slate-300">
                                        @if (isset($topic->created_at))
                                            {{ $topic->created_at->format('F j, Y') }}
                                        @endif
                                    </p>
                                </td>
                                <td class="px-5 py-5 border-b border-gray-200 bg-white dark:bg-slate-700 dark:border-gray-800 text-sm">
                                    ...
                                    @if(Auth::user()->getRoleAttribute() == 'Administrator' || auth()->user()->getRoleAttribute() == 'Tutor')
                                        ...
                                        <div class="flex">
                                            <a href="{{ route('topic.destroy', $topic->id) }}" >
                                                <form id="delete-form" action="{{ route('topic.destroy', $topic->id) }}" method="POST" >
                                                    @csrf
                                                    @method('DELETE')
