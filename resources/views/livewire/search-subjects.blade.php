<div>
    <div class=" flex items-center justify-between pb-6">
        <div>
            <h2 class="text-gray-600 dark:text-gray-200 font-semibold">Subjects</h2>
            <span class="text-xs dark:text-slate-400">All subject items</span>
        </div>
                ...
                <div class="lg:ml-40 ml-10 space-x-8">
                    @if(Auth::user()->getRoleAttribute() == 'Administrator')
                        <a href="{{route('subject.create')}}">
                            <button type="button" class="mt-6 text-white bg-gradient-to-r from-teal-400 via-teal-500 to-teal-600 hover:bg-gradient-to-br focus:ring-4 focus:outline-none focus:ring-teal-300 dark:focus:ring-teal-800 shadow-lg shadow-teal-500/50 dark:shadow-lg dark:shadow-teal-800/80 font-medium rounded-lg text-sm px-5 py-2.5 text-center me-2 mb-2">Add new subject</button>
                        </a>
                    @endif
                </div>
            </div>
                    ...
                    <table class="min-w-full leading-normal">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td class="px-5 py-5 border-b border-gray-200 bg-white dark:bg-slate-700 dark:border-gray-800 text-sm">
                                    <p class="text-gray-900 whitespace-no-wrap dark:text-slate-300">@if (isset($subject->description)) {{$subject->description}} @endif</p>
                                </td>
                                <td class="px-5 py-5 border-b border-gray-200 bg-white dark:bg-slate-700 dark:border-gray-800 text-sm">
                                    <p class="text-gray-900 whitespace-no-wrap dark:text-slate-300">
                                        @if (isset($subject->created_at))
                                            {{ $subject->created_at->format('F j, Y') }}
                                        @endif
                                    </p>
                                </td>
                                <td class="px-5 py-5 border-b border-gray-200 bg-white dark:bg-slate-700 dark:border-gray-800 text-sm">
                                    ...
                                    @if(Auth::user()->getRoleAttribute() == 'Administrator')
                                        ...
                                        <div class="flex">
                                            <a href="{{ route('subject.destroy', $subject->id) }}" >
                                                <form id="delete-form" action="{{ route('subject.destroy', $subject->id) }}" method="POST" >
                                                    @csrf
                                                    @method('DELETE')
