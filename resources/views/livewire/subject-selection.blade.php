<div class="bg-white  dark:bg-gray-800 overflow-hidden shadow-xl sm:rounded-lg px-8 pb-8">
    ...
    <div class="mt-8 grid grid-cols-2 gap-6 md:grid-cols-4">
        ...
        @foreach ($subjects as $subject)
            <span wire:click="selectSubject({{ $subject->id }})" class="group relative grid w-full min-w-[7rem] transform cursor-pointer place-items-center rounded-xl border border-blue-gray-50 bg-white dark:bg-slate-600 dark:border-slate-500 px-3 py-2 transition-all hover:scale-105 hover:border-blue-gray-100 hover:bg-blue-gray-50 hover:bg-opacity-25">
                <span class="my-6 grid h-24 w-24 place-items-center">
                    ...
                    <img class="rounded-t-lg" src="{{url('/images/students-subject-icon.webp')}}" alt="online-register" />
                    <span class="block mt-2 mb-2 text-sm font-medium text-gray-900 dark:text-white">@if (isset($subject->title)) {{$subject->title}} @endif</span>
